...
function createApp({
  ...
  const redis = redisClient || createRedisClient({ url: env.REDIS_URL, connectTimeoutMs: policy.redisConnectTimeoutMs });
  const dedupe = dedupeStore || createDedupeStore({ redisClient: redis, namespace: policy.redisNamespace, ttlSeconds: policy.dedupeTtlSeconds });
  const reservation = reservationStore || createReservationStore({ redisClient: redis, namespace: policy.redisNamespace, ttlSeconds: policy.reservationTtlSeconds });
  const cooldown = cooldownStore || createCooldownStore({ redisClient: redis, namespace: policy.redisNamespace, windowSeconds: policy.cooldownSeconds });
  ...
  const assistant = assistantPipeline || createAssistantPipeline({ skillRegistry: skills, publisher: telegramPublisher, conversationStore: memory, logger: log });
  ...
  async function syncMessages({ periodHours = 720, limitPerSource = 100, sources } = {}) {
    ...
    const collected = await telegramResearch.collect({ periodHours, limitPerSource, sources });
    const stored = await messageStore.upsertMany(collected.results || []);
    return {
      ok: true,
      sources: collected.sources || [],
      collected: Number(collected.count || 0),
      stored: Number(stored.count || 0),
      errors: collected.errors || [],
    };
  }

  app.get("/api/telegram-research/setup/status", telegramSetupGuard, async (_req, res) => runTelegramSetupAction(res, () => telegramResearch.status()));
  app.post("/api/telegram-research/setup/begin", telegramSetupGuard, async (req, res) => runTelegramSetupAction(res, () => telegramResearch.begin(req.body?.phone)));
  app.post("/api/telegram-research/setup/code", telegramSetupGuard, async (req, res) => runTelegramSetupAction(res, () => telegramResearch.code(req.body?.code)));
  app.post("/api/telegram-research/setup/password", telegramSetupGuard, async (req, res) => runTelegramSetupAction(res, () => telegramResearch.password(req.body?.password)));
  app.post("/api/telegram-research/setup/reset", telegramSetupGuard, async (_req, res) => runTelegramSetupAction(res, () => telegramResearch.reset()));

  app.get("/api/telegram-research/sources", telegramSetupGuard, async (_req, res) => runTelegramSetupAction(res, () => telegramResearch.listSources()));
  app.post("/api/telegram-research/sources", telegramSetupGuard, async (req, res) => runTelegramSetupAction(res, () => telegramResearch.addSource(req.body?.source)));
  app.delete("/api/telegram-research/sources/:source", telegramSetupGuard, async (req, res) => runTelegramSetupAction(res, () => telegramResearch.deleteSource(req.params.source)));
  ...
  app.post("/api/telegram-research/search", telegramSetupGuard, async (req, res) => {
    ...
    const sources = Array.isArray(req.body?.sources) ? req.body.sources.map((item) => String(item || "").trim()).filter(Boolean).slice(0, 50) : undefined;
  ...
  app.post("/api/telegram-research/ai-search", telegramSetupGuard, async (req, res) => {
    ...
    if (preview) {
      await runTelegramSetupAction(res, () => planTelegramQueries({ provider, goal, languages, model: policy.openaiChatModel }));
      return;
    }
    await runTelegramSetupAction(res, () => runTelegramAiSearch({ provider, telegramResearch, goal, languages, periodHours, limit, model: policy.openaiChatModel }));
  });

  app.get("/api/telegram-research/message-store/status", telegramSetupGuard, async (_req, res) => {
    res.json(await messageStore.status());
  });

  app.post("/api/telegram-research/message-store/sync", telegramSetupGuard, async (req, res) => {
    try {
      ...
      const sources = Array.isArray(req.body?.sources) ? req.body.sources.map((item) => String(item || "").trim()).filter(Boolean).slice(0, 50) : undefined;
      res.json(await syncMessages({ periodHours, limitPerSource, sources }));
    } catch (error) {
      res.status(error?.message === "MESSAGE_STORE_NOT_CONFIGURED" ? 503 : 502).json({ error: error?.message || "MESSAGE_STORE_SYNC_FAILED" });
    }
  });

  app.post("/api/telegram-research/deep-search", telegramSetupGuard, async (req, res) => {
    try {
      const query = String(req.body?.query || "").trim();
      if (!query) return res.status(400).json({ error: "SEARCH_QUERY_REQUIRED" });
      const sources = Array.isArray(req.body?.sources) ? req.body.sources : [];
      res.json(await messageStore.search({
        query,
        limit: Math.max(1, Math.min(100, Number(req.body?.limit || 30))),
        periodHours: Math.max(1, Math.min(24 * 3650, Number(req.body?.periodHours || 24 * 90))),
        threshold: Number(req.body?.threshold || 0.48),
        sources,
      }));
    } catch (error) {
      const status = error?.message === "MESSAGE_STORE_NOT_CONFIGURED" ? 503 : 400;
      res.status(status).json({ error: error?.message || "DEEP_SEARCH_FAILED" });
    }
  });
  ...
  app.post("/telegram/webhook", async (req, res) => {
    ...
    if (expectedSecret && receivedSecret !== expectedSecret) {
      log.warn({ traceId: null, reasonCode: "WEBHOOK_SECRET_INVALID", conversationId: null, messageId: null, userId: null });
    ...
    try {
      ...
      if (route.action !== "REPLY") {
        log.info({ traceId: event.traceId, reasonCode: route.reason, conversationId: event.conversationId, messageId: event.messageId, userId: event.userId });
        return;
      }
      const owner = checkOwner(event, policy.ownerUserId);
      if (!owner.allowed) {
        log.warn({ traceId: event.traceId, reasonCode: owner.reasonCode, conversationId: event.conversationId, messageId: event.messageId, userId: event.userId });
        return;
      }
...
async function start() {
  ...
  try { await redisClient.connect(); }
    ...
    logger.critical({ traceId: null, reasonCode: "REDIS_UNAVAILABLE", conversationId: null, messageId: null, userId: null, extra: { stage: "startup", error: err.message } });
  }

  if (messageStore.isConfigured()) {
    messageStore.init()
  ...
  app.listen(port, async () => {
    logger.info({ traceId: null, reasonCode: "SERVER_STARTED", conversationId: null, messageId: null, userId: null, extra: { port } });
    ...
    const syncEnabled = messageStore.isConfigured() && String(process.env.MESSAGE_SYNC_ENABLED || "true").toLowerCase() !== "false";
    if (syncEnabled) {
      const intervalSeconds = Math.max(900, Number(process.env.MESSAGE_SYNC_INTERVAL_SECONDS || 3600));
      const periodHours = Math.max(24, Number(process.env.MESSAGE_SYNC_PERIOD_HOURS || 720));
      const limitPerSource = Math.max(10, Math.min(200, Number(process.env.MESSAGE_SYNC_LIMIT_PER_SOURCE || 100)));
      const runSync = async () => {
        try {
          const result = await syncMessages({ periodHours, limitPerSource });
          logger.info({ traceId: null, reasonCode: "MESSAGE_STORE_SYNC_DONE", conversationId: null, messageId: null, userId: null, extra: { collected: result.collected, stored: result.stored, sources: result.sources.length, errors: result.errors.length } });
        } catch (error) {
          logger.error({ traceId: null, reasonCode: "MESSAGE_STORE_SYNC_FAILED", conversationId: null, messageId: null, userId: null, extra: { error: error?.message || String(error) } });
        }
      };
      setTimeout(runSync, 30000).unref();
      setInterval(runSync, intervalSeconds * 1000).unref();
    }
  });
}
